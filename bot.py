...
async def choose_budget_and_ask_line(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        if "skip" in callback.data:
            ...
            # 1. Получаем номер бюджета из callback_data ("budget_1" -> "1")
            budget_num = callback.data.split("_")[1]
            
            # 2. Маппинг: переводим цифру кнопки в точный текст, как в Supabase
            budget_mapping = {
                "1": "низкий",
                "2": "средний",
                "3": "высокий"
            ...
            # Получаем слово. Если вдруг прилетит неверный номер, запишется None
            budget_value = budget_mapping.get(budget_num)
        
        # 3. Сохраняем в состояние строковое значение для базы данных
        await state.update_data(budget=budget_value)
        print(f"Успешно сохранен текстовый бюджет для БД: {budget_value}")
...
async def finish_search_and_show_results(message: types.Message, state: FSMContext):
    ...
    for place in places[:5]:
        ...
        budget_val = place.get('budget_level')
        if budget_val == "низкий": budget_display = "$ Низкий"
        elif budget_val == "средний": budget_display = "$$ Средний"
        elif budget_val == "высокий": budget_display = "$$$ Высокий"
        else: budget_display = "Не указан"
